...
public class PrimaryOptionsProvider extends ThemeComponentOptionProvider<PrimaryOption> {
    ...
    protected void loadOptions() {
        ...
        Configuration lightConf = new Configuration(configuration);
        lightConf.uiMode &= ~Configuration.UI_MODE_NIGHT_MASK;
        lightConf.uiMode |= Configuration.UI_MODE_NIGHT_NO;
        Context lightContext = mContext.createConfigurationContext(lightConf);
        Configuration darkConf = new Configuration(configuration);
        darkConf.uiMode &= ~Configuration.UI_MODE_NIGHT_MASK;
        darkConf.uiMode |= Configuration.UI_MODE_NIGHT_YES;
        Context darkContext = mContext.createConfigurationContext(darkConf);
        for (String overlayPackage : mOverlayPackages) {
            try {
                Resources overlayResLight = getModifiedOverlayResources(lightContext, overlayPackage);
                Resources overlayResDark = getModifiedOverlayResources(darkContext, overlayPackage);
                Configuration llconf = overlayResLight.getConfiguration();
                // REMOVE ME
                String llconfmode = (llconf.uiMode & Configuration.UI_MODE_NIGHT_MASK) == Configuration.UI_MODE_NIGHT_YES ? "DARK" : "LIGHT";
                Log.e(TAG, "LIGHT?: " + llconfmode);
                llconf.uiMode &= ~Configuration.UI_MODE_NIGHT_MASK;
                llconf.uiMode |= Configuration.UI_MODE_NIGHT_NO;
                llconfmode = (llconf.uiMode & Configuration.UI_MODE_NIGHT_MASK) == Configuration.UI_MODE_NIGHT_YES ? "DARK" : "LIGHT";
                Log.e(TAG, "LIGHT2?: " + llconfmode);
                Configuration dlconf = overlayResDark.getConfiguration();
                String dlconfmode = (dlconf.uiMode & Configuration.UI_MODE_NIGHT_MASK) == Configuration.UI_MODE_NIGHT_YES ? "DARK" : "LIGHT";
                Log.e(TAG, "DARK?: " + dlconfmode);
                dlconf.uiMode &= ~Configuration.UI_MODE_NIGHT_MASK;
                dlconf.uiMode |= Configuration.UI_MODE_NIGHT_YES;
                dlconfmode = (dlconf.uiMode & Configuration.UI_MODE_NIGHT_MASK) == Configuration.UI_MODE_NIGHT_YES ? "DARK" : "LIGHT";
                Log.e(TAG, "DARK2?: " + dlconfmode);
    ...
    protected Resources getModifiedOverlayResources(Context context, String overlayPackage) throws NameNotFoundException {
        return context.getPackageManager().getResourcesForApplication(overlayPackage);
    }
}
